import type { CharacterSet } from "../types.ts";
import { BidiClass, Category, CharacterSetType } from "../enums.ts";

/**
 * _Unicode Dataset:_ **Enclosed Ideographic Supplement**
 *
 * _Unicode Version:_ **17.0.0**
 *
 * _Start Code:_ **127488**
 *
 * _End Code:_ **127743**
 *
 * _Total Characters:_ **64**
 */
export const dataSet: CharacterSet = {
  blockName: CharacterSetType.Enclosed_Ideographic_Supplement,
  startCode: 127488,
  endCode: 127743,
  characters: [
    {
      code: 127488,
      name: "SQUARE HIRAGANA HOKA",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12411, 12363],
      decompType: "square",
    },
    {
      code: 127489,
      name: "SQUARED KATAKANA KOKO",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12467, 12467],
      decompType: "square",
    },
    {
      code: 127490,
      name: "SQUARED KATAKANA SA",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12469],
      decompType: "square",
    },
    {
      code: 127504,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-624B",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [25163],
      decompType: "square",
    },
    {
      code: 127505,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5B57",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [23383],
      decompType: "square",
    },
    {
      code: 127506,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-53CC",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21452],
      decompType: "square",
    },
    {
      code: 127507,
      name: "SQUARED KATAKANA DE",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12487],
      decompType: "square",
    },
    {
      code: 127508,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-4E8C",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [20108],
      decompType: "square",
    },
    {
      code: 127509,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-591A",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [22810],
      decompType: "square",
    },
    {
      code: 127510,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-89E3",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [35299],
      decompType: "square",
    },
    {
      code: 127511,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5929",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [22825],
      decompType: "square",
    },
    {
      code: 127512,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-4EA4",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [20132],
      decompType: "square",
    },
    {
      code: 127513,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6620",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [26144],
      decompType: "square",
    },
    {
      code: 127514,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-7121",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [28961],
      decompType: "square",
    },
    {
      code: 127515,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6599",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [26009],
      decompType: "square",
    },
    {
      code: 127516,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-524D",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21069],
      decompType: "square",
    },
    {
      code: 127517,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5F8C",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [24460],
      decompType: "square",
    },
    {
      code: 127518,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-518D",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [20877],
      decompType: "square",
    },
    {
      code: 127519,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-65B0",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [26032],
      decompType: "square",
    },
    {
      code: 127520,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-521D",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21021],
      decompType: "square",
    },
    {
      code: 127521,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-7D42",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [32066],
      decompType: "square",
    },
    {
      code: 127522,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-751F",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [29983],
      decompType: "square",
    },
    {
      code: 127523,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-8CA9",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [36009],
      decompType: "square",
    },
    {
      code: 127524,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-58F0",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [22768],
      decompType: "square",
    },
    {
      code: 127525,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5439",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21561],
      decompType: "square",
    },
    {
      code: 127526,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6F14",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [28436],
      decompType: "square",
    },
    {
      code: 127527,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6295",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [25237],
      decompType: "square",
    },
    {
      code: 127528,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6355",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [25429],
      decompType: "square",
    },
    {
      code: 127529,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-4E00",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [19968],
      decompType: "square",
    },
    {
      code: 127530,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-4E09",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [19977],
      decompType: "square",
    },
    {
      code: 127531,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-904A",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [36938],
      decompType: "square",
    },
    {
      code: 127532,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5DE6",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [24038],
      decompType: "square",
    },
    {
      code: 127533,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-4E2D",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [20013],
      decompType: "square",
    },
    {
      code: 127534,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-53F3",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21491],
      decompType: "square",
    },
    {
      code: 127535,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6307",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [25351],
      decompType: "square",
    },
    {
      code: 127536,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-8D70",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [36208],
      decompType: "square",
    },
    {
      code: 127537,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6253",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [25171],
      decompType: "square",
    },
    {
      code: 127538,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-7981",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [31105],
      decompType: "square",
    },
    {
      code: 127539,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-7A7A",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [31354],
      decompType: "square",
    },
    {
      code: 127540,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5408",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21512],
      decompType: "square",
    },
    {
      code: 127541,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6E80",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [28288],
      decompType: "square",
    },
    {
      code: 127542,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6709",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [26377],
      decompType: "square",
    },
    {
      code: 127543,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-6708",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [26376],
      decompType: "square",
    },
    {
      code: 127544,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-7533",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [30003],
      decompType: "square",
    },
    {
      code: 127545,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-5272",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21106],
      decompType: "square",
    },
    {
      code: 127546,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-55B6",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21942],
      decompType: "square",
    },
    {
      code: 127547,
      name: "SQUARED CJK UNIFIED IDEOGRAPH-914D",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [37197],
      decompType: "square",
    },
    {
      code: 127552,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-672C",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 26412, 12309],
      decompType: "compat",
    },
    {
      code: 127553,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-4E09",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 19977, 12309],
      decompType: "compat",
    },
    {
      code: 127554,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-4E8C",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 20108, 12309],
      decompType: "compat",
    },
    {
      code: 127555,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-5B89",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 23433, 12309],
      decompType: "compat",
    },
    {
      code: 127556,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-70B9",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 28857, 12309],
      decompType: "compat",
    },
    {
      code: 127557,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-6253",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 25171, 12309],
      decompType: "compat",
    },
    {
      code: 127558,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-76D7",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 30423, 12309],
      decompType: "compat",
    },
    {
      code: 127559,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-52DD",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 21213, 12309],
      decompType: "compat",
    },
    {
      code: 127560,
      name: "TORTOISE SHELL BRACKETED CJK UNIFIED IDEOGRAPH-6557",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [12308, 25943, 12309],
      decompType: "compat",
    },
    {
      code: 127568,
      name: "CIRCLED IDEOGRAPH ADVANTAGE",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [24471],
      decompType: "circle",
    },
    {
      code: 127569,
      name: "CIRCLED IDEOGRAPH ACCEPT",
      cat: Category.Other_Symbol,
      bidi: BidiClass.Left_To_Right,
      decomp: [21487],
      decompType: "circle",
    },
    { code: 127584, name: "ROUNDED SYMBOL FOR FU", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
    { code: 127585, name: "ROUNDED SYMBOL FOR LU", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
    { code: 127586, name: "ROUNDED SYMBOL FOR SHOU", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
    { code: 127587, name: "ROUNDED SYMBOL FOR XI", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
    { code: 127588, name: "ROUNDED SYMBOL FOR SHUANGXI", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
    { code: 127589, name: "ROUNDED SYMBOL FOR CAI", cat: Category.Other_Symbol, bidi: BidiClass.Other_Neutral },
  ],
};
